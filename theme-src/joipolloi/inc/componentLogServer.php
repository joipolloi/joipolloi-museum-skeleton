...
namespace Joi\ComponentLogServer;

use Joi;

define(__NAMESPACE__ . '\NS', __NAMESPACE__ . '\\');

add_action('Joi/afterRegisterComponents', function () {
    if (((defined('WP_ENV') && WP_ENV === 'development') || current_user_can('editor') || current_user_can('administrator')) && isset($_GET['log'])) {
        ...
        add_filter("Joi/addComponentData", NS . 'addDebugInfo', 99999, 2);
    }
}, 11);
...
function addDebugInfo($data, $componentName)
{
    if (
        ...
        in_array($componentName, Joi\ComponentLogServer\COMPONENT_WHITELIST)
    ) {
        consoleDebug([
